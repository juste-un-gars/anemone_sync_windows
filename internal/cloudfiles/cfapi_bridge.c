...
static void CALLBACK OnCancelFetchDataCallback(
    ...
    (void)callbackParameters; // unused
    if (!g_initialized) return;
...
int32_t CfapiBridgeInit(void) {
    if (g_initialized) {
        return CFAPI_BRIDGE_OK; // Already initialized
    }
...
int32_t CfapiBridgeDisconnect(int64_t connectionKey) {
    ...
    if (FAILED(hr)) {
        fprintf(stderr, "[CFAPI Bridge] CfDisconnectSyncRoot failed: 0x%08lX\n", hr);
        return CFAPI_BRIDGE_ERROR_API_FAILED;
    }
...
int32_t CfapiBridgeTransferData(
    ...
    if (FAILED(hr)) {
        fprintf(stderr, "[CFAPI Bridge] CfExecute (TransferData) failed: 0x%08lX\n", hr);
        return CFAPI_BRIDGE_ERROR_API_FAILED;
    }
...
int32_t CfapiBridgeTransferComplete(
    ...
    HRESULT hr = g_pfnCfExecute(&opInfo, &opParams);
    if (FAILED(hr)) {
        fprintf(stderr, "[CFAPI Bridge] CfExecute (AckData) failed: 0x%08lX\n", hr);
        return CFAPI_BRIDGE_ERROR_API_FAILED;
    }
...
int32_t CfapiBridgeTransferError(
    ...
    HRESULT hr = g_pfnCfExecute(&opInfo, &opParams);
    if (FAILED(hr)) {
        fprintf(stderr, "[CFAPI Bridge] CfExecute (TransferError) failed: 0x%08lX\n", hr);
        return CFAPI_BRIDGE_ERROR_API_FAILED;
    }
